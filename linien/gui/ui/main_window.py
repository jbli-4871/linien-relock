...
class MainWindow(QtWidgets.QMainWindow, CustomWidget):
    ...
    def ready(self):
        ...
        # Error DC Offset slider is added but not connected to any functionality yet
        self.ids.error_dc_offset_slider.setValue(0)
    ...
    def connection_established(self):
        ...
        def change_manual_navigation_visibility(*args):
            ...
            relock_running = getattr(params, "relock_running", None)
            rl_running = relock_running.value if relock_running is not None else False
            optimization = params.optimization_running.value
            locked = params.lock.value

            busy = al_running or rl_running or optimization

            self.get_widget('manual_navigation').setVisible(not busy and not locked)
            self.get_widget('top_lock_panel').setVisible(locked)
            self.get_widget('statusbar_unlocked').setVisible(not busy and not locked)
        ...
        params.relock_running.on_change(change_manual_navigation_visibility)
